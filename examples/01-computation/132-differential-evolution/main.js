// Differential Evolution - Main Thread
...
const mutationFactorInput = document.getElementById('mutationFactor');
const crossoverProbInput = document.getElementById('crossoverProb');
...
function updateProgress(data) {
    ...
    progressText.textContent = `Gen ${data.generation} | Best: ${data.bestFitness.toExponential(4)} | Avg: ${data.avgFitness.toExponential(4)} | Success: ${data.successRate}%`;
}
...
function drawPopulation(data) {
    ...
    popCtx.fillStyle = '#140a0f'; popCtx.fillRect(0, 0, w, h);
    ...
    // Grid
    popCtx.strokeStyle = '#201418'; popCtx.lineWidth = 1;
    for (let i = 0; i <= 10; i++) {
    ...
    // Labels
    popCtx.fillStyle = '#8a5a6a'; popCtx.font = '10px monospace'; popCtx.textAlign = 'center';
    for (let i = 0; i <= 4; i++) {
    ...
    popCtx.beginPath();
    popCtx.arc(mapX(bestPosition[0]), mapY(bestPosition[1]), 9, 0, Math.PI * 2);
    popCtx.fill();
    ...
    // Border
    popCtx.strokeStyle = '#3a2a2e'; popCtx.lineWidth = 2; popCtx.strokeRect(p, p, pw, ph);
...
function drawFitnessHistory(data) {
    ...
    fitCtx.fillStyle = '#140a0f'; fitCtx.fillRect(0, 0, w, h);
    ...
    // Grid
    fitCtx.strokeStyle = '#201418'; fitCtx.lineWidth = 1;
    for (let i = 0; i <= 4; i++) {
        fitCtx.beginPath(); fitCtx.moveTo(p, 10 + i / 4 * ph); fitCtx.lineTo(w - 10, 10 + i / 4 * ph); fitCtx.stroke();
    }

    // Average fitness
    fitCtx.strokeStyle = '#f9a8d4'; fitCtx.lineWidth = 1; fitCtx.beginPath();
    ...
    // Best fitness
    fitCtx.strokeStyle = '#f472b6'; fitCtx.lineWidth = 2; fitCtx.beginPath();
    ...
    // Legend
    fitCtx.fillStyle = '#f9a8d4'; fitCtx.font = '9px sans-serif';
    fitCtx.fillText('Avg', p + 5, 18);
    fitCtx.fillStyle = '#f472b6';
...
function drawDiversityHistory(data) {
    ...
    divCtx.fillStyle = '#140a0f'; divCtx.fillRect(0, 0, w, h);
    ...
    // Grid
    divCtx.strokeStyle = '#201418'; divCtx.lineWidth = 1;
    for (let i = 0; i <= 4; i++) {
        divCtx.beginPath(); divCtx.moveTo(p, 10 + i / 4 * ph); divCtx.lineTo(w - 10, 10 + i / 4 * ph); divCtx.stroke();
    }

    // Diversity curve
    divCtx.strokeStyle = '#ec4899'; divCtx.lineWidth = 2; divCtx.beginPath();
    ...
    // Labels
    divCtx.fillStyle = '#8a5a6a'; divCtx.font = '9px monospace'; divCtx.textAlign = 'right';
...
function startEvolution() {
    const params = {
        ...
        mutationFactor: parseFloat(mutationFactorInput.value),
        crossoverProb: parseFloat(crossoverProbInput.value),
...
function reset() {
    ...
    popCtx.fillStyle = '#140a0f'; popCtx.fillRect(0, 0, populationCanvas.width, populationCanvas.height);
    popCtx.fillStyle = '#8a5a6a'; popCtx.font = '14px sans-serif'; popCtx.textAlign = 'center';
    popCtx.fillText('Click "Start DE" to begin', populationCanvas.width / 2, populationCanvas.height / 2);
    fitCtx.fillStyle = '#140a0f'; fitCtx.fillRect(0, 0, fitnessCanvas.width, fitnessCanvas.height);
    divCtx.fillStyle = '#140a0f'; divCtx.fillRect(0, 0, diversityCanvas.width, diversityCanvas.height);
}
