// BFGS Quasi-Newton - Main Thread

const functionSelect = document.getElementById('functionSelect');
const maxIterInput = document.getElementById('maxIter');
const toleranceInput = document.getElementById('tolerance');
const lineSearchType = document.getElementById('lineSearchType');
...
function drawPath(data) {
    ...
    if (path.length > 1) {
        for (let i = 1; i < path.length; i++) {
            ...
            optimCtx.strokeStyle = `rgba(52, 211, 153, ${0.3 + t * 0.7})`;
        ...
        for (let i = 0; i < path.length; i++) {
            optimCtx.fillStyle = `rgba(167, 243, 208, ${0.4 + (i / path.length) * 0.6})`;
            optimCtx.beginPath();
...
function startOptimization() {
    const params = {
        ...
        lineSearchType: lineSearchType.value,
...
function reset() {
    ...
    optimCtx.textAlign = 'center';
    optimCtx.fillText('Click "Start BFGS" to begin', optimCanvas.width / 2, optimCanvas.height / 2);
    lossCtx.fillStyle = '#0a1612';
