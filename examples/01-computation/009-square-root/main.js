    ...
    elements.calculateResult.innerHTML = `
        ...
        <div class="result-content">
            <div class="result-formula">
                √${escapeHtml(number)} = ${escapeHtml(result)}
            </div>
...
function displayVerifyResult(data) {
    ...
    const { number, squareRoot, squared, error, precision, iterations, time, isAccurate } = data;
    ...
    elements.verifyResult.innerHTML = `
        ...
        <div class="result-content">
            ...
            <div class="verify-steps">
                ...
                <div class="verify-step">
                    <span class="step-label">√${escapeHtml(number)} =</span>
                    <span class="step-value result-box">${escapeHtml(squareRoot)}</span>
                </div>
                <div class="verify-step">
                    <span class="step-label">(√${escapeHtml(number)})² =</span>
                    <span class="step-value result-box">${escapeHtml(squared)}</span>
                </div>
                <div class="verify-step">
    ...
    elements.compareResult.innerHTML = `
        ...
        <div class="result-content">
            <div class="compare-header">
                <p>計算 √${escapeHtml(number)} 精度 ${precision} 位</p>
            </div>
            ...
            <div class="compare-summary">
                <h4>效能分析</h4>
                <ul>
                    <li><strong>牛頓迭代法</strong>：收斂最快，適合大多數場景</li>
                    <li><strong>二分搜尋法</strong>：穩定但較慢，適合驗證</li>
                    <li><strong>數位逐位法</strong>：直觀但效率較低</li>
                </ul>
            </div>
        </div>
    `;
...
function validateInput(value, type = 'number') {
    if (type === 'number') {
        ...
        if (value.trim().startsWith('-')) {
            return { valid: false, error: '無法計算負數的平方根' };
        }
