...
document.addEventListener('DOMContentLoaded', function() {
    ...
    worker.onmessage = function(e) {
        ...
        if (type === 'PROGRESS') {
            ...
            `;
            html += '<h3>Column Order</h3>';
            html += `<p>Key order: ${payload.order.map(o => o + 1).join(', ')}</p>`;
            if (payload.grid) {
                html += '<h3>Grid View</h3>';
                html += '<div style="overflow-x:auto"><table style="border-collapse:collapse;font-family:monospace">';
                html += '<tr>';
                for (let i = 0; i < payload.key.length; i++) {
                    html += `<th style="padding:8px;border:1px solid #ddd;background:#e3f2fd">${payload.key[i]}<br><small>${payload.order.indexOf(i) + 1}</small></th>`;
                }
                html += '</tr>';
                for (const row of payload.grid) {
                    html += '<tr>';
                    for (const cell of row) {
                        html += `<td style="padding:8px;border:1px solid #ddd;text-align:center">${cell || ''}</td>`;
                    }
                    html += '</tr>';
                }
                html += '</table></div>';
...
function processText() {
    ...
    worker.postMessage({
        type: 'PROCESS',
        payload: {
            text: elements.inputText.value.toUpperCase().replace(/[^A-Z]/g, ''),
            key: elements.inputKey.value.toUpperCase(),
