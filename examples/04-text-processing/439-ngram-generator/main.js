    ...
    worker.onmessage = function(e) {
        ...
        if (type === 'PROGRESS') {
            ...
            elements.resultStats.innerHTML = `
                ...
                <div class="stat-item"><span class="stat-label">N-grams:</span><span class="stat-value">${payload.stats.count}</span></div>
            `;
            elements.resultOutput.innerHTML = payload.ngrams.map(n =>
                `<div class="stat-item"><span class="stat-value">"${n.gram}"</span><span class="stat-label">${n.count}x</span></div>`
            ).join('');
...
function processText() {
    ...
    worker.postMessage({ type: 'GENERATE', payload: { text: elements.inputText.value, n: parseInt(elements.ngramSize.value) } });
}
