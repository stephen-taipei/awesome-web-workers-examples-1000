...
document.addEventListener('DOMContentLoaded', function() {
    ...
    elements.variantSelect = document.getElementById('variant-select');
    ...
    worker.onmessage = function(e) {
        ...
        if (type === 'PROGRESS') {
            ...
            elements.resultStats.innerHTML = `
                ...
                <div class="stat-item"><span class="stat-label">Variant:</span><span class="stat-value">${payload.variant.toUpperCase()}</span></div>
                <div class="stat-item"><span class="stat-label">Characters Changed:</span><span class="stat-value">${payload.changed}</span></div>
            `;
            let html = `
                <h3>Original</h3>
            ...
            `;
            html += `<p style="color:#666;margin-top:15px"><small>Note: ${payload.variant.toUpperCase()} is symmetric - applying it twice returns the original text.</small></p>`;

            // Show alphabet mapping for ROT13
            if (payload.variant === 'rot13') {
                ...
                for (let i = 0; i < 26; i++) {
                    html += `<div style="width:24px;text-align:center;padding:4px;background:#e3f2fd;border-radius:2px">${String.fromCharCode(65 + ((i + 13) % 26))}</div>`;
                }
                html += '</div></div>';
            }
...
function processText() {
    ...
    worker.postMessage({
        ...
        payload: {
            ...
            variant: elements.variantSelect.value
        }
    });
}
