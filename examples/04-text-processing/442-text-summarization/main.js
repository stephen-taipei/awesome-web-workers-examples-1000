...
const sentenceCountSlider = document.getElementById('sentenceCount');
const sentenceCountVal = document.getElementById('sentenceCountVal');
...
function initWorker() {
    ...
    worker.onmessage = function(e) {
        const { type, summary, time } = e.data;
        if (type === 'result') {
            summaryOutput.innerHTML = summary.map(s => `<p>• ${s}</p>`).join('');
            timeTaken.textContent = `(耗時: ${time.toFixed(2)}ms)`;
            processBtn.disabled = false;
            processBtn.textContent = '生成摘要';
        }
    };
}

sentenceCountSlider.oninput = () => {
    sentenceCountVal.textContent = sentenceCountSlider.value;
};
...
function process() {
    ...
    worker.postMessage({
        text,
        count: parseInt(sentenceCountSlider.value)
    });
}
