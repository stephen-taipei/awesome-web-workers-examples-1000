...
const tempShiftEl = document.getElementById('tempShift');
...
function showResult(data) {
    ...
    const shift = data.temperature;
    if (shift > 0) {
        tempShiftEl.textContent = `+${shift} (Warmer)`;
    } else if (shift < 0) {
        tempShiftEl.textContent = `${shift} (Cooler)`;
    } else {
        tempShiftEl.textContent = `0 (Neutral)`;
    }
...
function loadImage(file) {
    ...
    reader.onload = function(e) {
        ...
        img.onload = function() {
            ...
            resultCtx.fillStyle = '#fbbf24';
            resultCtx.font = '14px sans-serif';
            resultCtx.textAlign = 'center';
            resultCtx.fillText('Click "Apply Temperature" to process', width / 2, height / 2);
...
function reset() {
    ...
    if (currentImage && originalImageData) {
        ...
        resultCtx.font = '14px sans-serif';
        resultCtx.textAlign = 'center';
        resultCtx.fillText('Click "Apply Temperature" to process', resultCanvas.width / 2, resultCanvas.height / 2);
    }
...
function downloadResult() {
    ...
    const suffix = temp > 0 ? `warm${temp}` : temp < 0 ? `cool${Math.abs(temp)}` : 'neutral';
    link.download = `color-temp-${suffix}.png`;
...
tempSlider.addEventListener('input', function() {
    ...
    if (val > 0) {
        tempValue.textContent = `+${val} (Warmer)`;
    } else if (val < 0) {
        tempValue.textContent = `${val} (Cooler)`;
    } else {
        tempValue.textContent = '0 (Neutral)';
    }
});
