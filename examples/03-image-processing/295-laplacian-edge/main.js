/**
 * Laplacian 邊緣檢測 - 主執行緒腳本
 */

let worker = null;
let originalImageData = null;
let currentKernel = 'basic';
...
function initializeElements() {
    ...
    elements.kernelBtns = document.querySelectorAll('[data-kernel]');
...
function setupEventListeners() {
    ...
    elements.kernelBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            elements.kernelBtns.forEach(b => b.classList.remove('active'));
...
function loadDemoImage() {
    ...
    ctx.font = '30px Arial';
    ctx.fillText('Laplacian', 250, 250);
...
function downloadResult() {
    ...
    link.download = 'laplacian-result.png';
...
function displayResult(result) {
    ...
    elements.resultStats.innerHTML = `
        ...
        <div class="stat-item"><span class="stat-label">核心：</span><span class="stat-value">${result.kernelType}</span></div>
    `;
}
