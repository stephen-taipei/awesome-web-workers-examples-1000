...
searchInput.addEventListener('input', () => searchValue.textContent = searchInput.value);
...
function processImage() {
    ...
    worker.postMessage({
        ...
        searchSize: parseInt(searchInput.value)
    });
    worker.onmessage = (e) => {
        if (e.data.type === 'progress') {
            const p = Math.round(e.data.progress * 100);
