...
const numSuperpixelsValue = document.getElementById('numSuperpixelsValue');
...
const superpixelCount = document.getElementById('superpixelCount');
const worker = new Worker('worker.js');

uploadArea.addEventListener('click', () => fileInput.click());
fileInput.addEventListener('change', (e) => loadImage(e.target.files[0]));
processBtn.addEventListener('click', processImage);
numSuperpixelsInput.addEventListener('input', () => numSuperpixelsValue.textContent = numSuperpixelsInput.value);
compactnessInput.addEventListener('input', () => compactnessValue.textContent = compactnessInput.value);

worker.onmessage = (e) => {
    resultCtx.putImageData(e.data.imageData, 0, 0);
    superpixelCount.textContent = e.data.count;
