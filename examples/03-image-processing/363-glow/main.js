...
function initWorker() {
    ...
    worker.onmessage = function(e) {
        const { type, data, duration, progress, status } = e.data;

        if (type === 'progress') {
            const percent = Math.round(progress * 100);
            progressBar.style.width = `${percent}%`;
            progressText.textContent = `${status || '處理中'}... ${percent}%`;
        } else if (type === 'result') {
...
radiusInput.addEventListener('input', (e) => radiusVal.textContent = e.target.value);
intensityInput.addEventListener('input', (e) => intensityVal.textContent = e.target.value);
