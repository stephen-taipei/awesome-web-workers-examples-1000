...
function loadDemoImage() {
    ...
    ctx.fillText('RADIAL', cx, 150);
    ctx.fillText('BLUR', cx, 250);
...
function applyBlur() {
    ...
    worker.postMessage({
        ...
        payload: {
            imageData: imageData,
            cx: cx, // normalized 0-1
            cy: cy, // normalized 0-1
            strength: strength,
            samples: samples
        }
...
function downloadResult() {
    ...
    link.download = 'radial-blurred.png';
...
function displayResult(result) {
    ...
    elements.resultStats.innerHTML = `
        ...
        <div class="stat-item"><span class="stat-label">強度 / 採樣：</span><span class="stat-value">${result.strength}° / ${result.samples}</span></div>
    `;
>>>>>>> origin/main
}
