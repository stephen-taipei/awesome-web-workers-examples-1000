...
const elements = {
    ...
    fileInput: null,
    loadDemoBtn: null,
    sigmaSpaceInput: null,
    sigmaSpaceValue: null,
    sigmaColorInput: null,
    sigmaColorValue: null,
    applyBtn: null,
    resetBtn: null,
    downloadBtn: null,
    progressBar: null,
...
function initializeElements() {
    ...
    elements.sigmaSpaceInput = document.getElementById('sigma-space-input');
    elements.sigmaSpaceValue = document.getElementById('sigma-space-value');
    elements.sigmaColorInput = document.getElementById('sigma-color-input');
    elements.sigmaColorValue = document.getElementById('sigma-color-value');
...
function setupEventListeners() {
    ...
    elements.sigmaSpaceInput.addEventListener('input', updateParameterDisplay);
    elements.sigmaColorInput.addEventListener('input', updateParameterDisplay);
...
function loadDemoImage() {
    ...
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.arc(200, 180, 20, 0.2, Math.PI - 0.2);
    ctx.stroke();

    // 添加 "斑點" (噪聲)
    for(let i=0; i<300; i++) {
        const x = 200 + (Math.random()-0.5) * 120;
        const y = 150 + (Math.random()-0.5) * 160;
        if ((x-200)**2/6400 + (y-150)**2/10000 <= 1) { // 橢圓內
             ctx.fillStyle = `rgba(160, 100, 80, 0.2)`;
             ctx.beginPath();
    ...
    ctx.fillText('Bilateral Filter', 200, 280);
...
function updateParameterDisplay() {
    elements.sigmaSpaceValue.textContent = elements.sigmaSpaceInput.value;
    elements.sigmaColorValue.textContent = elements.sigmaColorInput.value;
}
...
function applyFilter() {
    ...
    const sigmaSpace = parseFloat(elements.sigmaSpaceInput.value);
    const sigmaColor = parseFloat(elements.sigmaColorInput.value);
...
function downloadResult() {
    ...
    link.download = 'bilateral-filtered.png';
...
function displayResult(result) {
    ...
    elements.resultStats.innerHTML = `
        ...
        <div class="stat-item"><span class="stat-label">Space σ / Color σ：</span><span class="stat-value">${result.sigmaSpace} / ${result.sigmaColor}</span></div>
    `;
}
