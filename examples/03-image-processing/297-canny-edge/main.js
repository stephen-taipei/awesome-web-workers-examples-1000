...
function initializeElements() {
    ...
    elements.highThreshold = document.getElementById('high-threshold');
    elements.highVal = document.getElementById('high-val');
    elements.lowThreshold = document.getElementById('low-threshold');
    elements.lowVal = document.getElementById('low-val');
...
function setupEventListeners() {
    ...
    elements.sigmaInput.addEventListener('input', () => elements.sigmaValue.textContent = elements.sigmaInput.value);

    elements.highThreshold.addEventListener('input', () => {
        let high = parseInt(elements.highThreshold.value);
        let low = parseInt(elements.lowThreshold.value);
        if (high < low) {
            elements.lowThreshold.value = high;
            elements.lowVal.textContent = high;
        }
        elements.highVal.textContent = high;
    });

    elements.lowThreshold.addEventListener('input', () => {
        let high = parseInt(elements.highThreshold.value);
        let low = parseInt(elements.lowThreshold.value);
        if (low > high) {
            elements.highThreshold.value = low;
            elements.highVal.textContent = low;
        }
        elements.lowVal.textContent = low;
    });

    elements.applyBtn.addEventListener('click', applyCanny);
...
function applyCanny() {
    ...
    worker.postMessage({
        ...
        payload: {
            imageData,
            sigma: parseFloat(elements.sigmaInput.value),
            highThreshold: parseInt(elements.highThreshold.value),
            lowThreshold: parseInt(elements.lowThreshold.value)
        }
...
function downloadResult() {
    ...
    link.download = 'canny-result.png';
...
function displayResult(result) {
    ...
    elements.resultStats.innerHTML = `
        ...
        <div class="stat-item"><span class="stat-label">Sigma：</span><span class="stat-value">${result.sigma}</span></div>
        <div class="stat-item"><span class="stat-label">雙閾值：</span><span class="stat-value">H:${result.highThreshold} / L:${result.lowThreshold}</span></div>
    `;
}
