/**
 * Column Processor - Main Thread Script
 */
...
function processData(data) {
    ...
    if (items.length === 0) return { output: {}, stats: { count: 0, columns: 0 } };
    const columns = {};
    const keys = Object.keys(items[0]);
    keys.forEach(key => {
        columns[key] = { values: items.map(row => row[key]), type: typeof items[0][key] };
    });
    return { output: columns, stats: { count: items.length, columns: keys.length } };
}
