/**
 * Patch Applier - Main Thread Script
 */
...
function processData(data) {
    ...
    const patches = items.slice(1).flat().filter(p => p && p.op);
    const result = { ...target };
    patches.forEach(p => {
        if (p.op === 'add' || p.op === 'replace') result[p.path] = p.value;
        if (p.op === 'remove') delete result[p.path];
    });
    return { output: result, stats: { count: patches.length, processed: patches.length } };
}
