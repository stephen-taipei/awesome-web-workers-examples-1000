/**
 * Conflict Resolver - Main Thread Script
 */
...
function processData(data) {
    ...
    const resolved = items.map(item => {
        if (item.conflict) {
            return { ...item, resolved: item.priority === 'ours' ? item.ours : item.theirs };
        }
        return item;
    });
    return { output: resolved, stats: { count: items.length, processed: resolved.length } };
}
