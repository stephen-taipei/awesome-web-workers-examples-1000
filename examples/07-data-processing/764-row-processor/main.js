/**
 * Row Processor - Main Thread Script
 */
...
function processData(data) {
    ...
    const rows = items.map((item, i) => {
        ...
        return { rowIndex: i, columnCount: values.length, values, keys: Object.keys(item) };
    });
    return { output: rows, stats: { count: items.length, processed: rows.length } };
}
