/**
 * ETL Pipeline - Main Thread Script
 */
...
function processData(data) {
    ...
    const transformed = extracted.map(item => ({ ...item, transformed: true, timestamp: Date.now() }));
    const loaded = transformed;
    return { output: { stages: { extracted: extracted.length, transformed: transformed.length, loaded: loaded.length }, sample: loaded.slice(0, 5) }, stats: { count: items.length, processed: loaded.length } };
}
