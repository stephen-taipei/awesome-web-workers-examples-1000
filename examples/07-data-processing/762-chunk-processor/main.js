/**
 * Chunk Processor - Main Thread Script
 */
...
function processData(data) {
    ...
    for (let i = 0; i < items.length; i += chunkSize) {
        chunks.push({ chunkId: chunks.length, data: items.slice(i, i + chunkSize) });
    }
    return { output: chunks, stats: { count: items.length, chunks: chunks.length } };
}
