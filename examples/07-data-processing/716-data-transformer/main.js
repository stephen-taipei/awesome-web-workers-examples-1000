/**
 * Data Transformer - Main Thread Script
 */
...
function processData(data) {
    ...
    const transformed = items.map(item => {
        const result = { ...item };
        if (item.name) result.name_upper = item.name.toUpperCase();
        if (item.value !== undefined) result.value_doubled = item.value * 2;
        result.processed_at = new Date().toISOString();
        return result;
    });
    return { output: transformed, stats: { count: items.length, processed: transformed.length } };
}
