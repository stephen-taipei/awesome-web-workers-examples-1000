/**
 * Reservoir Sampler - Main Thread Script
 */
...
function processData(data) {
    ...
    const k = Math.min(10, Math.max(1, Math.floor(items.length * 0.1)));
    const reservoir = [];
    ...
    return { output: reservoir, stats: { count: items.length, sampled: reservoir.length, sampleRate: (reservoir.length / items.length * 100).toFixed(1) + '%' } };
}
