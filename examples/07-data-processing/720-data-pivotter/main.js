/**
 * Data Pivotter - Main Thread Script
 */
...
function processData(data) {
    ...
    const pivot = {};
    items.forEach(item => {
        const row = item.category || 'default';
        const col = item.name || 'value';
        if (!pivot[row]) pivot[row] = {};
        pivot[row][col] = (pivot[row][col] || 0) + (item.value || 1);
    });
    return { output: pivot, stats: { count: items.length, rows: Object.keys(pivot).length } };
}
