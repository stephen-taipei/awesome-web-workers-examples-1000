/**
 * Batch Processor - Main Thread Script
 */
...
function processData(data) {
    ...
    for (let i = 0; i < items.length; i += batchSize) {
        ...
        batches.push({ batchId: Math.floor(i / batchSize), size: batch.length, items: batch });
    }
    return { output: batches, stats: { count: items.length, batches: batches.length } };
}
