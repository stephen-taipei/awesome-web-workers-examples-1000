...
function processData(data) {
    ...
    const decoded = items.map(item => {
        const result = { ...item };
        if (typeof item.encoded === 'number' && Array.isArray(data.categories)) {
            result.decoded = data.categories[item.encoded];
        }
        return result;
    });
    return { output: decoded, stats: { count: items.length, processed: decoded.length } };
}
