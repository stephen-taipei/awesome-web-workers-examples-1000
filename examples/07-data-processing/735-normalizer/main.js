...
function processData(data) {
    ...
    const normalized = values.map(v => ({ original: v, normalized: (v - min) / range }));
    return { output: normalized, stats: { count: values.length, min, max, processed: normalized.length } };
}
