/**
 * Data Sorter - Main Thread Script
 */
...
function processData(data) {
    const items = Array.isArray(data) ? data.slice() : [data];
    items.sort((a, b) => {
        const va = typeof a === 'number' ? a : (a.value || 0);
        const vb = typeof b === 'number' ? b : (b.value || 0);
        return va - vb;
    });
    return { output: items, stats: { count: items.length, processed: items.length } };
}
