/**
 * #507 Loop Benchmark - Main Thread
 */
...
function runBenchmark() {
    ...
    if (isNaN(iterations) || iterations < 10000) {
        showError('Please enter at least 10000 iterations');
        return;
    }
    hideError();
    isRunning = true;
...
function displayResults(r) {
    ...
    elements.resultStats.innerHTML = `
        <div class="stat-item"><span class="stat-label">For Loop:</span><span class="stat-value">${r.forTime.toFixed(2)} ms</span></div>
        <div class="stat-item"><span class="stat-label">While Loop:</span><span class="stat-value">${r.whileTime.toFixed(2)} ms</span></div>
        <div class="stat-item"><span class="stat-label">ForEach:</span><span class="stat-value">${r.forEachTime.toFixed(2)} ms</span></div>
        <div class="stat-item"><span class="stat-label">For...of:</span><span class="stat-value">${r.forOfTime.toFixed(2)} ms</span></div>
        <div class="stat-item"><span class="stat-label">Reduce:</span><span class="stat-value">${r.reduceTime.toFixed(2)} ms</span></div>
